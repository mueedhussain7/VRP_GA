...
# Function to plot vehicle routes
def plot_routes(instance, routes, save_path):
    ...
    colors = cm.get_cmap('tab20').colors  # supports multiple colors
...
n_runs = 5  # number of runs per instance+param

# Run GA for all instances and parameters
for name, inst in instances.items():
    ...
    for param in params:
        ...
        for _ in range(n_runs):
            best_ind, best_cost, best_routes, best_cost_per_gen = solve_ga(inst, **param)
...
# Plot best cost vs population size
...
print("\nAll experiments completed. All plots saved in 'results/' folder.")
